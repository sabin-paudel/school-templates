          ...
          <div className="space-y-0">
            ...
            {notices.map((notice, index) => (
              <Reveal key={notice.title} delay={index * 0.05}>
                <article className="bg-white rounded-xl p-6 shadow-sm hover:shadow-md transition-all duration-300 mb-4">
                  <div className="flex flex-wrap items-center gap-3 text-xs font-medium uppercase tracking-wider">
                    ...
                    {notice.pinned && (
                      <span className="flex items-center gap-1 bg-primary-light px-2 py-0.5 text-primary rounded">
                        <Pin size={11} /> Pinned
                      </span>
                    )}
                  </div>
